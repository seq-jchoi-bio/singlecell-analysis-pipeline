...
# load UMAP from CSV (FIG_DIR/umap.csv)
...
# Heatmap loader (robust z-score CSV)
from typing import List

def _load_heatmap_robustz_csv(base_dir: str):
    import os
    import pandas as pd
    csv_path = os.path.join(base_dir, "Annot_results", "Plots", "heatmap_robustz.csv")
    if not os.path.exists(csv_path):
        return None, []
    df = pd.read_csv(csv_path, index_col=0)

    groups = []
    if df.shape[0] > 0 and str(df.index[0]) == "clusterName":
        labels = df.loc["clusterName"].astype(str).fillna("")
        ordered_cols: List[str] = []
        for lab in sorted(pd.unique(labels)):
            genes = [c for c in df.columns if str(labels.get(c, "")) == lab]
            if genes:
                groups.append((lab, genes))
                ordered_cols.extend(genes)
        df = df.drop(index=["clusterName"], errors="ignore")
        if ordered_cols:
            df = df[ordered_cols]
    else:
        groups = [("all", df.columns.tolist())]

    df = df.apply(pd.to_numeric, errors="coerce")
    return df, groups

def _plotly_heatmap(z, x_labels=None, y_labels=None, height=480, groups=None) -> str:
    ...
    if groups:
        ...
        group_ids = []
        labels_by_pos = []
        counts_by_label = {lab: len(genes) for lab, genes in groups}
        for gid, (lab, genes) in enumerate(groups):
            for g in genes:
                ordered.append(g)
                group_ids.append(gid)
                labels_by_pos.append(lab)
        if x_labels is not None:
            idx = {lbl: i for i, lbl in enumerate(x)}
            cols = [idx[g] for g in ordered if g in idx]
            if len(cols) == z.shape[1]:
                z = z[:, cols]
                x = [x[i] for i in cols]
                group_ids = [group_ids[i] for i in range(len(cols))]
                labels_by_pos = [labels_by_pos[i] for i in range(len(cols))]
        fig = make_subplots(rows=2, cols=1, shared_xaxes=True, row_heights=[0.06, 0.94], vertical_spacing=0.02)
        fig_width = 1400  # fixed width so each cell gets thinner as N increases
        ...
        nuniq = len(set(group_ids)) if group_ids else 0
        colorscale = [[i/(max(nuniq-1,1)), base_colors[i % len(base_colors)]] for i in range(max(nuniq,1))]
        strip_z = np.asarray([group_ids], dtype=float)
        strip_text = [f"{lab} (n={counts_by_label.get(lab,0)})" for lab in labels_by_pos]
        fig.add_trace(
            go.Heatmap(z=strip_z, x=x, y=["clusterName"], colorscale=colorscale, showscale=False,
                       text=strip_text, hovertemplate="<b>%{text}</b><extra></extra>"),
            row=1, col=1
        )
        # Main heatmap (hover shows cluster + count)
        counts_by_pos = [counts_by_label.get(lbl, 0) for lbl in labels_by_pos]
        cd_main = np.stack([
            np.tile(np.asarray(labels_by_pos)[None, :], (z.shape[0], 1)),
            np.tile(np.asarray(counts_by_pos)[None, :], (z.shape[0], 1))
        ], axis=-1)
        fig.add_trace(
            go.Heatmap(z=z, x=x, y=y, colorscale="RdBu", reversescale=True, zmid=0,
                       colorbar=dict(title="robustz", thickness=12, len=0.85, y=0.52), customdata=cd_main, text=x,
                       hovertemplate="gene=%{x}<br>row=%{y}<br>cluster=%{customdata[0]} (n=%{customdata[1]})<br>z=%{z}<extra></extra>"),
            row=2, col=1
        )
        # Hide x ticks; slightly tighter margins
        fig.update_yaxes(showticklabels=False, row=1, col=1)
        fig.update_xaxes(showticklabels=False, row=1, col=1)
        fig.update_xaxes(showticklabels=False, row=2, col=1)
        fig.update_xaxes(title_text="Gene", row=2, col=1)
        fig.update_yaxes(title_text="Leiden", row=2, col=1)
        # Group separators
        cum = 0
        for lab, genes in groups:
            cum += len(genes)
            if cum < len(x):
                fig.add_vline(x=cum-0.5, line_width=1, line_color="white", row=2, col=1)
        fig.update_layout(margin=dict(l=60, r=6, t=24, b=60), height=height, width=fig_width,
...
# render API
def render_report(summary_text: Optional[str] = None,
                  umap: Optional[Dict[str, Sequence[Any]]] = None,
                  heatmap: Optional[Dict[str, Any]] = None,
                  base_dir: str = "Test",
                  out_name: str = "annot_summary.html",
                  title: str = "Annotation Summary",
                  **kwargs: Any) -> str:
    ...
    if heatmap and ("z" in heatmap):
        heat_html = _plotly_heatmap(heatmap["z"], heatmap.get("x_labels"), heatmap.get("y_labels"), height=480, groups=None)
    else:
        _df, _groups = _load_heatmap_robustz_csv(base_dir)
        if _df is not None and not _df.empty:
            heat_html = _plotly_heatmap(_df.values, list(_df.columns), list(_df.index), height=480, groups=_groups)
