...
def safe_write_h5ad(adata, path):
    # Ensure obs is pandas.DataFrame and dtypes are sane before writing
    import pandas as pd
    try:
        obs_df = _ensure_obs_dataframe(adata.obs, index=getattr(adata, 'obs_names', None))
        # dtype normalization
        for col in list(obs_df.columns):
            if col in ("depth","n_fragment"):
                obs_df[col] = pd.to_numeric(obs_df[col], errors="coerce")
        adata.obs = obs_df
    except Exception as _e:
        # In worst case, keep existing obs and continue; writing may still succeed
        pass
    try:
        adata.write(path)
    except Exception as _e:
        # Retry by stripping non-pandas obs entirely (emergency fallback)
        try:
            adata.obs = pd.DataFrame(index=pd.Index(getattr(adata, 'obs_names', [])))
            adata.write(path)
...
# === Safe pandas coercion helpers (robust to PyDataFrame/PyDataFrameElem) ===
def _ensure_pandas_series(obj):
    import pandas as pd
    # direct pandas
    ...
    if hasattr(obj, "to_pandas"):
        try:
            ...
            if isinstance(ser, pd.DataFrame):
                # take first column if a DataFrame sneaks in
                if ser.shape[1] == 1:
                    return ser.iloc[:, 0]
                return pd.Series(ser.iloc[:, 0])
            return pd.Series(ser) if not isinstance(ser, pd.Series) else ser
        except Exception:
            pass
    # numpy array-like
    try:
        ...
        if hasattr(obj, "__array__"):
            return pd.Series(_np.asarray(obj))
    except Exception:
        pass
    # mapping with 'values'
    try:
...
def _ensure_obs_dataframe(obs_like, index=None):
    import pandas as pd
    # if already pandas DataFrame
    if type(obs_like).__module__.startswith("pandas") and hasattr(obs_like, "columns"):
        df = obs_like.copy()
    else:
        # try to_pandas
        if hasattr(obs_like, "to_pandas"):
            try:
                tmp = obs_like.to_pandas()
        ...
        if df is None:
            # common column access path
            cols = getattr(obs_like, "columns", None)
            if cols is not None:
                try:
                    data = {str(c): _ensure_pandas_series(obs_like[c]) for c in list(cols)}
                    df = pd.DataFrame(data)
                except Exception:
                    df = None
        if df is None:
            # generic frame conversion
            try:
                df = pd.DataFrame(obs_like)
            except Exception:
                # last resort empty df with only index
                df = pd.DataFrame()
...
# In-memory report cache (UMAP, heatmap)
_REPORT_CACHE = {}
...
def build_gene_activity_fallback(data: snap.AnnDataSet, gene_gtf: str, flank_up=2000, flank_down=0) -> an.AnnData:
    ...
    ga = an.AnnData(
        X=X_gene,
        obs=_ensure_obs_dataframe(ad.obs, index=ad.obs_names if hasattr(ad, 'obs_names') else None),
...
def main():
    ...
    # UMAP + plots
    if SAVE_UMAP:
        _snap.tl.umap(data, use_rep="X_spectral_harmony", use_dims=USE_DIMS)
        try:
    ...
    try:
        ...
        cluster_csv=os.path.join(AGGR_DIR, 'Plots', 'cluster_assignments.csv')
                ...
                # ---- Write annot_merged_cells.h5ad (final state) ----
                try:
                    import numpy as _np
                    import anndata as _ad
                    _out_h5ad = os.path.join(AGGR_DIR, "annot_merged_cells.h5ad")

                    # Helpers to assign obs safely as string/float arrays
                    def _assign_str_obs(_adset, _target, _candidates):
                        for _k in _candidates:
                            try:
                                if _k in _adset.obs:
                                    _vals = _obs_as_str_array(_adset, _k)
                                    _adset.obs[_target] = _np.asarray(_vals, dtype=object)
                                    return True
                            except Exception:
                                pass
                        return False
                    ...
                    if "sample" in data.obs:
                        try:
                            data.obs["sample"] = _np.asarray(_obs_as_str_array(data, "sample"), dtype=object)
                        except Exception:
                            pass
                    else:
                        if not _assign_str_obs(data, "sample", ["batch","library","dataset"]):
                            data.obs["sample"] = _np.asarray(["NA"] * data.n_obs, dtype=object)

                    # leiden
                    if "leiden" in data.obs:
                        try:
                            data.obs["leiden"] = _np.asarray(_obs_as_str_array(data, "leiden"), dtype=object)
                        except Exception:
                            pass

                    # majority
                    if "majority" in data.obs:
                        try:
                            data.obs["majority"] = _np.asarray(_obs_as_str_array(data, "majority"), dtype=object)
                        except Exception:
                            pass
                    else:
                        if not _assign_str_obs(data, "majority", ["best_label","pred_cell_type","leiden_majority_clusterName","clusterName"]):
                            if "leiden" in data.obs:
                                data.obs["majority"] = _np.asarray(_obs_as_str_array(data, "leiden"), dtype=object)
                            else:
                                data.obs["majority"] = _np.asarray(["NA"] * data.n_obs, dtype=object)

                    # depth (numeric)
                    try:
                        _vals, _ = _obs_numeric_depth_series(data)
                        data.obs["depth"] = _np.asarray(_vals, dtype=float)
                    except Exception:
                        if "n_fragment" in data.obs:
                            try:
                                v = _np.log10(_np.asarray(data.obs["n_fragment"], dtype=float) + 1.0)
                            except Exception:
                                v = _np.asarray([0.0]*data.n_obs, dtype=float)
                            data.obs["depth"] = v
                        elif "depth" not in data.obs:
                            data.obs["depth"] = _np.asarray([0.0]*data.n_obs, dtype=float)
                    ...
                    if "X_umap" not in data.obsm:
                        try:
                            ...
                            print(f"[{ts()}] Warning: cannot create X_umap on the fly: {_e}")

                    # Create minimal AnnData (obs+obsm only)
                    _A = _ad.AnnData(
                        X=_np.zeros((data.n_obs, 1), dtype=float),
                        obs=_ensure_obs_dataframe(data.obs, index=data.obs_names if hasattr(data, 'obs_names') else None),
                        obsm={"X_umap": _np.asarray(data.obsm["X_umap"]) if "X_umap" in data.obsm else _np.zeros((data.n_obs,2))}
                    )
                    safe_write_h5ad(_A, _out_h5ad)
                    print(f"[{ts()}] Wrote final h5ad:", _out_h5ad)
                except Exception as _e:
                    print(f"[{ts()}] Warning: failed to write annot_merged_cells.h5ad: {_e}")
                ...
                # ---- cache for report: UMAP + labels + heatmap (use SAME helpers as plotting) ----
                try:
                    import numpy as _np
                    if "X_umap" in data.obsm:
                        _X = _np.asarray(data.obsm["X_umap"])
                        _umap = {"x": _X[:,0].tolist(), "y": _X[:,1].tolist()}

                        # Use plotting helpers to ensure values/ordering identical to saved PNGs
                        try:
                            _sample_vec = _obs_as_str_array(data, "sample")
                            _umap["sample"] = list(map(str, _np.asarray(_sample_vec, dtype=object).tolist()))
                        except Exception:
                            _umap["sample"] = ["NA"] * data.n_obs

                        try:
                            _leiden_vec = _obs_as_str_array(data, "leiden")
                            _umap["leiden"] = list(map(str, _np.asarray(_leiden_vec, dtype=object).tolist()))
                        except Exception:
                            _umap["leiden"] = ["NA"] * data.n_obs

                        try:
                            _vals, _lbl = _obs_numeric_depth_series(data)
                            _umap["depth"] = list(map(float, _np.asarray(_vals).tolist()))
                        except Exception:
                            _umap["depth"] = [0.0] * data.n_obs

                        try:
                            _maj_vec = _obs_as_str_array(data, "majority")
                            _umap["majority"] = list(map(str, _np.asarray(_maj_vec, dtype=object).tolist()))
                        except Exception:
                            # fallback to best_label if majority is unavailable
                            try:
                                _maj_vec = _obs_as_str_array(data, "best_label")
                                _umap["majority"] = list(map(str, _np.asarray(_maj_vec, dtype=object).tolist()))
                            except Exception:
                                _umap["majority"] = _umap.get("leiden", ["NA"] * data.n_obs)

                        _REPORT_CACHE["umap"] = _umap
                except Exception as _e:
                    print(f"[{ts()}] Warning: failed to cache UMAP for report: {_e}")

                try:
                    _csv = os.path.join(FIG_DIR, "heatmap_reference_markers_robustz_cluster_median.csv")
                    if os.path.exists(_csv):
                        import pandas as _pd
                        _df = _pd.read_csv(_csv, index_col=0)
                        _REPORT_CACHE["heatmap"] = {
                            "z": _df.values.tolist(),
                            "x_labels": list(map(str, _df.columns.tolist())),
                            "y_labels": list(map(str, _df.index.tolist())),
                        }
                except Exception as _e:
                    print(f"[{ts()}] Warning: failed to cache heatmap for report: {_e}")

                # ---- cache for report: UMAP + labels + heatmap ----
                try:
                    import numpy as _np
                    if "X_umap" in data.obsm:
                        _X = _np.asarray(data.obsm["X_umap"])
                        ...
                        # sample
                        try:
                            _umap["sample"] = list(map(str, data.obs["sample"].tolist()))
                        except Exception:
                            _umap["sample"] = ["NA"] * data.n_obs
                        # leiden
                        try:
                            _umap["leiden"] = list(map(str, data.obs["leiden"].tolist()))
                        except Exception:
                            _umap["leiden"] = ["NA"] * data.n_obs
                        # depth numeric (same helper as plotting)
                        try:
                            vals, _lbl = _obs_numeric_depth_series(data)
                            _umap["depth"] = list(map(float, _np.asarray(vals).tolist()))
                        except Exception:
                            _umap["depth"] = [0.0] * data.n_obs
                        # majority (optional)
                        try:
                            _umap["majority"] = list(map(str, data.obs["majority"].tolist()))
                        except Exception:
                            _umap["majority"] = _umap.get("leiden", ["NA"] * data.n_obs)
                        _REPORT_CACHE["umap"] = _umap
                except Exception as _e:
...
# === Auto Report Hook (atexit) ===
# This hook triggers the HTML report generation after renew completes.
# Note: It does NOT change existing logic; it only runs at normal process exit.
import atexit, subprocess, os


def _make_report_after_exit():
    try:
        import os, sys, importlib.util, numpy as _np
        base_dir = os.path.dirname(AGGR_DIR)  # e.g., "Test"
        summary_path = os.path.join(AGGR_DIR, "summary.txt")
        if os.path.exists(summary_path):
            ...
            _summary = ""
        # Prefer loading from final annot_merged_cells.h5ad
        _h5ad_path = os.path.join(AGGR_DIR, "annot_merged_cells.h5ad")
        if os.path.exists(_h5ad_path):
            try:
                import anndata as _ad
                import numpy as _np
                _A = _ad.read_h5ad(_h5ad_path)
                _umap = {}
                _heat = {}
                if "X_umap" in _A.obsm:
                    _X = _np.asarray(_A.obsm["X_umap"])
                    _umap["x"] = _X[:,0].tolist()
                    _umap["y"] = _X[:,1].tolist()
                try: _umap["sample"] = list(map(str, _A.obs["sample"].tolist()))
                except Exception: pass
                try: _umap["leiden"] = list(map(str, _A.obs["leiden"].tolist()))
                except Exception: pass
                try: _umap["majority"] = list(map(str, _A.obs.get("majority", _A.obs.get("best_label")).tolist()))
                except Exception: pass
                try:
                    _umap["depth"] = _A_ensure_pandas_series(.obs["depth"]).astype(float).tolist()
                except Exception:
                    try:
                        _umap["depth"] = _np.asarray(_A.obs["n_fragment"], dtype=float).tolist()
                    except Exception:
                        _umap["depth"] = [0.0] * _A.n_obs
                print(f"[{ts()}] Using annot_merged_cells.h5ad for report payload.")
            except Exception as _e:
                print(f"[{ts()}] Warning: failed to read h5ad for report: {_e}")


        # Resolve path of makeAnnotReport.py robustly (no __file__ dependency)
        try:
            _script_dir = os.path.dirname(os.path.abspath(__file__))
        except NameError:
            _argv0 = sys.argv[0] if sys.argv and sys.argv[0] else ""
            _script_dir = os.path.dirname(os.path.abspath(_argv0)) if _argv0 else os.getcwd()

        _candidates = [
            os.path.join(_script_dir, "makeAnnotReport.py"),
            os.path.join(os.getcwd(), "makeAnnotReport.py"),
            os.path.join(base_dir, "makeAnnotReport.py"),
        ]
        _report_py = None
        for _p in _candidates:
            if os.path.exists(_p):
                _report_py = _p
                break
        if _report_py is None:
            raise RuntimeError("makeAnnotReport.py not found near renew. Checked: " + str(_candidates))

        spec = importlib.util.spec_from_file_location("makeAnnotReport_local", _report_py)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)  # must expose render_report(...)

        # Prefer cached payload from main()
        if "_umap" not in locals() or not _umap:
            _umap = dict(_REPORT_CACHE.get("umap", {}))
        if "_heat" not in locals() or not _heat:
            _heat = dict(_REPORT_CACHE.get("heatmap", {}))

        # If cache missing, try to build UMAP payload from in-memory object
        if not _umap:
            _g = globals()
            _ad = _g.get("data", None) or _g.get("adata", None)
            try:
                if _ad is not None and "X_umap" in _ad.obsm:
                    _X = _np.asarray(_ad.obsm["X_umap"])
                    _umap["x"] = _X[:, 0].tolist()
                    _umap["y"] = _X[:, 1].tolist()
                    try:
                        _umap["sample"] = list(map(str, _ad.obs["sample"].tolist()))
                    except Exception:
                        _umap["sample"] = ["NA"] * _ad.n_obs
                    try:
                        _umap["leiden"] = list(map(str, _ad.obs["leiden"].tolist()))
                    except Exception:
                        _umap["leiden"] = ["NA"] * _ad.n_obs
                    try:
                        if "depth" in _ad.obs:
                            _umap["depth"] = _ad_ensure_pandas_series(.obs["depth"]).astype(float).tolist()
                        elif "n_fragment" in _ad.obs:
                            _umap["depth"] = _ad_ensure_pandas_series(.obs["n_fragment"]).astype(float).tolist()
                        else:
                            _umap["depth"] = [0.0] * _ad.n_obs
                    except Exception:
                        _umap["depth"] = [0.0] * _ad.n_obs
                    try:
                        _umap["majority"] = list(map(str, _ad.obs.get("majority", _ad.obs.get("best_label")).tolist()))
                    except Exception:
                        _umap["majority"] = _umap.get("leiden", ["NA"] * _ad.n_obs)
            except Exception as _e:
                print(f"[{ts()}] Warning: failed to assemble in-memory UMAP: {_e}")

        # If heatmap not cached, pick it up from the existing CSV
        if not _heat:
            try:
                _csv = os.path.join(FIG_DIR, "heatmap_reference_markers_robustz_cluster_median.csv")
                if os.path.exists(_csv):
                ...
                print(f"[{ts()}] Warning: failed to collect heatmap arrays: {_e}")

        # Heatmap from already-exported robustz median CSV (no new artifacts)
        ...
        out_html = mod.render_report(
            summary_text=_summary,
            umap=_umap,
            heatmap=_heat,
            base_dir=base_dir,
            out_name="annot_summary.html",
            title="Annotation Summary",
        )
        print("[INFO] Report generated successfully.", out_html)
    except Exception as e:
        print(f"[WARN] Report generation failed: {e}")

atexit.register(_make_report_after_exit)