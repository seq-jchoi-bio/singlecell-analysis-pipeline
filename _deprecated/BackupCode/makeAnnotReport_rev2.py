...
def _load_heatmap_robustz_csv(base_dir: str):
    """
    Read base_dir/Annot_results/Plots/heatmap_robustz.csv
    Returns: (df_numeric, groups)
    """
    import os
    import pandas as pd
...
def _plotly_heatmap(z, x_labels=None, y_labels=None, height=480, groups=None) -> str:
    ...
    if groups:
        # Build order & per-position labels
        ordered = []
        group_ids = []
        labels_by_pos = []
        ...
        # Top strip with hover showing cluster and count
        ...
        fig.add_trace(
            go.Heatmap(z=strip_z, x=x, y=["clusterName"], colorscale=colorscale, showscale=False,
                       text=strip_text, hovertemplate="%{text}<extra></extra>"),
            row=1, col=1
        )
        ...
        fig.add_trace(
            go.Heatmap(z=z, x=x, y=y, colorscale="RdBu", reversescale=True, zmid=0,
                       colorbar=dict(len=0.85, y=0.12), customdata=cd_main, text=x,
