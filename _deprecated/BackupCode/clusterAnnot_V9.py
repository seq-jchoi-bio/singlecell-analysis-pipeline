...
HEADER = (
    "=================================================================================\n"
    "  Integrated scATAC-seq: Clustering + Annotation + BED converting \n"
    "  Version 4.0-nobed \n"
    "================================================================================="
)
desc_txt = ""
...
def plot_marker_heatmap_and_export(gene_ad, ad_with_obs, marker_pairs: List[Tuple[str,str]], figdir: str, basename="heatmap_reference_markers",
    ...
    # --- Inject clusterName (reference label) as the first row for both CSVs ---
    gene_to_label = {}
    try:
...
# ---- Robust obs helpers for PyDataFrameElem/pandas ----
...
# ---- Full exporters: dump all obs columns + UMAP coordinates ----
...
# ---- OBS → DataFrame (full) + UMAP exporters ----
def _obs_to_full_dataframe(adata):
    ...
    # 1) Prefer pandas-based conversion
    try:
        import pandas as pd
    ...
    # 2) Manual fallback (no pandas dependency)
    try:
        # Collect keys
        keys = []
        try:
        ...
        try:
            ...
            pass
        # Force common columns if present
        ...
        # Write simple CSV by hand (will be wrapped by exporters)
    ...
    try:
        ...
        if isinstance(pack, dict) and pack.get("__manual_df__"):
            ...
            try:
                ...
                # Fallback to manual writer if pandas unexpectedly fails
                import csv
...
def _make_report_after_exit():
    try:
        ...
        # Try to build UMAP payload from in-memory object if available
        _umap = {}
        _g = globals()
