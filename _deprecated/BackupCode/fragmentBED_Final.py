...
# ----------------- small utils -----------------
def _pick_label_field(adata, preferred='majority'):
    import pandas as pd

    candidates = [preferred, 'best_label', 'leiden']
    ordered = []
    seen = set()
    for c in candidates:
        if c not in seen:
    ...
    for key in ordered:
        if key in adata.obs:
            ...
            try:
                ...
                pass
            # skip if single categorical category "NA"
            try:
                if getattr(s.dtype, "name", "") == "category":
                    cats = list(s.cat.categories)
                    if len(cats) == 1 and str(cats[0]).upper() == "NA":
                        continue
            except Exception:
                pass
            return key, s
    # fallback
    return preferred, adata.obs[ordered[-1]] if ordered[-1] in adata.obs else None
...
def _safe_label_string(x):
    s = "NA" if x is None else str(x).strip()
...
def update_and_export(
    ...
    bed_suffix: str = ".bed",
    convert_to_bed4: bool = False,
    # Defaults tuned for this project:
    cluster_csv: Optional[str] = "Annot_results/Plots/cluster_assignments.csv",
    label_key: str = "best_label",
    cells_assignments_csv: Optional[str] = None,
    aggr_h5ad: Optional[str] = "Annot_results/annot_gene_activity.h5ad",
) -> Dict[str, object]:
...
def cli():
    ...
    p.add_argument('--base_dir', default='.', help='Base directory (typically project root).')
    p.add_argument('--njob', type=int, default=8, help='Parallel jobs for per-sample update (default: 8).')
    p.add_argument("--h5ad_dir", required=True, help="Input directory containing per-sample *.h5ad")
    p.add_argument("--out_update_dir", default="Annot_results/update_h5ad", help="Directory to write updated_*.h5ad")
    p.add_argument("--out_bed_dir", default="Annot_results/bed", help="Directory to write BED files")
    p.add_argument("--summary", default="Annot_results/summary.txt", help="Path to append summary (summary.txt)")
    p.add_argument("--leiden_key", default="leiden")
    p.add_argument("--sample_key", default="sample")
    p.add_argument("--cluster_csv", default="Annot_results/Plots/cluster_assignments.csv", help="CSV with columns [leiden,best_label]")
    ...
    p.add_argument("--aggr_h5ad", default="Annot_results/annot_gene_activity.h5ad", help="Aggregated h5ad with obs['leiden']")
    # Back-compat flag; ignored
    p.add_argument("--majority_key", default="majority")
    args = p.parse_args()

    # Resolve all paths relative to base_dir if they are not absolute
    base = args.base_dir
    update_and_export(
        h5ad_dir=_resolve(args.h5ad_dir, base),
        out_update_dir=_resolve(args.out_update_dir, base),
        out_bed_dir=_resolve(args.out_bed_dir, base),
        njob=args.njob,
        summary_path=_resolve(args.summary, base),
        leiden_key=args.leiden_key,
        sample_key=args.sample_key,
        cluster_csv=_resolve(args.cluster_csv, base),
        label_key=args.label_key,
        cells_assignments_csv=_resolve(args.cells_assignments_csv, base),
        aggr_h5ad=_resolve(args.aggr_h5ad, base),
