...
desc_txt = """
------------
This script loads a pre-merged .h5ads dataset and constructs a gene activity matrix
based on gene IDs with robust GTF parsing. It then imports a reference CSV of marker
genes to define per-cell-type signatures and transfers cell-type labels.
The final outputs include annotated .h5ad and CSV files, UMAP visualizations, and per-cluster
BED files stored under BASE_DIR/bed as "<Label>_<Sample>.bed".
"""

# ---------------- User-configurable section (wrapper-injectable) ----------------
# Path
BASE_DIR        = "."
H5ADS_INPUT     = os.path.join(BASE_DIR, "Filter_results", "merged_doublets.h5ads")
AGGR_DIR        = os.path.join(BASE_DIR, "Annot_results")
...
SAVE_UMAP           = True
TSS_FLANK_UP   = int(os.environ.get('TSS_FLANK_UP', '2000'))  # promoter upstream flank
TSS_FLANK_DOWN = int(os.environ.get('TSS_FLANK_DOWN', '2000'))     # downstream flank

# Param for label assignment
...
def _apply_wrapper_injections_and_defaults() -> None:
    ...
    for key in ["H5ADS_INPUT","AGGR_DIR","FIG_DIR","H5ADS_OUT","ANNOT_H5AD","ANNOT_GENE_H5AD","SUMMARY_TXT","CSV_OUT"]:
        if key in globals_dict and isinstance(globals_dict[key], str):
            globals_dict[key] = _norm(globals_dict[key])
    # Ensure output dirs
    try:
        os.makedirs(globals_dict.get("AGGR_DIR",""), exist_ok=True)
    except Exception:
        pass
    try:
        os.makedirs(globals_dict.get("FIG_DIR",""), exist_ok=True)
    except Exception:
        pass
...
def plot_marker_heatmap_and_export(gene_ad, ad_with_obs, marker_pairs: List[Tuple[str,str]], figdir: str, basename="heatmap_reference_markers",
    ...
    if hasattr(medZ, "columns") and getattr(medZ.columns, "has_duplicates", False):
        medZ = medZ.T.groupby(level=0).mean().T
    if hasattr(meanLogCPM, "columns") and getattr(meanLogCPM.columns, "has_duplicates", False):
        meanLogCPM = meanLogCPM.T.groupby(level=0).mean().T
    ordered_all_genes = list(dict.fromkeys(ordered_all_genes))
    ...
    os.makedirs(figdir, exist_ok=True)
    csv_z  = os.path.join(figdir, "heatmap_robustz.csv")
    csv_cpm = os.path.join(figdir, "heatmap_fragcpm.csv")

    gene_to_label = {}
    try:
        for _g, _lab in marker_pairs:
            if _g not in gene_to_label:
                gene_to_label[_g] = _lab
    except Exception:
        pass

    _label_row = [str(gene_to_label.get(g, "")) for g in medZ.columns]

    medZ_out = medZ.copy()
    medZ_out.loc["clusterName"] = _label_row
    medZ_out = medZ_out.loc[["clusterName"] + [i for i in medZ.index]]

    meanLogCPM_out = meanLogCPM.copy()
    meanLogCPM_out.loc["clusterName"] = _label_row
    meanLogCPM_out = meanLogCPM_out.loc[["clusterName"] + [i for i in meanLogCPM.index]]

    medZ_out.to_csv(csv_z, index=True);  print(f"[{ts()}] Saved CSV: {csv_z}")
    meanLogCPM_out.to_csv(csv_cpm, index=True);  print(f"[{ts()}] Saved CSV: {csv_cpm}")
    ...
    _draw(medZ_f, "Robust z (cluster median)", "heatmap_robustz", clamp=clamp_robustz)
    _draw(meanLogCPM_f, "Pseudobulk accessibility (log1p, fragCPM)", "heatmap_fragcpm", clamp=clamp_logcpm)
    print(f"[{ts()}] Heatmaps + CSV export completed.")

def _obs_has(adata, key: str) -> bool:
    _obs = getattr(adata, "obs", None)
    if _obs is None:
        return False
    try:
        cols = getattr(_obs, "columns", None)
        if cols is not None:
            return key in cols
    except Exception:
        pass
    try:
        if hasattr(_obs, "keys") and key in list(_obs.keys()):
            return True
    except Exception:
        pass
    try:
...
def _col_to_list(col):
    try:
        ...
        pass
    for attr in ("to_list","to_pylist","to_numpy","values"):
        try:
            v = getattr(col, attr)
...
def _safe_obs_values(adata, key):
    if not key:
        return None
    try:
        col = _obs_get(adata, key)
    except Exception:
        return None
    return _col_to_list(col)

# Exporters
def _export_umap_csv(data, out_path: str, include_nfrag: bool = True):
    try:
        ...
        if um.shape[1] < 2:
            return False
        n = um.shape[0]
        sk = _first_key(data, ["sample","Sample","sample_name","sampleid","sample_id"])
        lk = _first_key(data, ["leiden","Leiden","cluster","Cluster"])
        nk = _first_key(data, ["n_fragment","n_fragments","passed_filters","nFragments","depth"]) if include_nfrag else None
        sample_vals = _safe_obs_values(data, sk) if sk else None
        leiden_vals = _safe_obs_values(data, lk) if lk else None
        nfrag_vals  = _safe_obs_values(data, nk) if nk else None
        os.makedirs(os.path.dirname(out_path), exist_ok=True)
        header = ["sample","leiden","n_fragment","umap_x","umap_y"]
        with open(out_path, "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(header)
            for i in range(n):
                s = str(sample_vals[i]) if sample_vals and i < len(sample_vals) else ""
                l = str(leiden_vals[i]) if leiden_vals and i < len(leiden_vals) else ""
                if nfrag_vals and i < len(nfrag_vals):
                    try: nf = float(nfrag_vals[i])
                    except Exception: nf = ""
                else:
                    nf = ""
                w.writerow([s, l, nf, float(um[i,0]), float(um[i,1])])
        print(f"[{ts()}] Saved CSV: {out_path}")
        return True
    except Exception as _e:
        print(f"[{ts()}] Warning: failed to write {out_path}: {_e}")
        return False

def _export_umap_majority_csv(data, out_path: str):
    """Export: sample, leiden, best_label, umap_x, umap_y (prefer 'majority')."""
    try:
        import numpy as np, csv, os
        if "X_umap" not in data.obsm:
            return False
        ...
        sk = _first_key(data, ["sample","Sample","sample_name","sampleid","sample_id"])
        lk = _first_key(data, ["leiden","Leiden","cluster","Cluster"])
        bk = _first_key(data, ["majority","best_label","Best_label","label"])
        sample_vals = _safe_obs_values(data, sk) if sk else None
        leiden_vals = _safe_obs_values(data, lk) if lk else None
        best_vals   = _safe_obs_values(data, bk) if bk else None
        os.makedirs(os.path.dirname(out_path), exist_ok=True)
        header = ["sample","leiden","best_label","umap_x","umap_y"]
        with open(out_path, "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(header)
            for i in range(n):
                s = str(sample_vals[i]) if sample_vals and i < len(sample_vals) else ""
                l = str(leiden_vals[i]) if leiden_vals and i < len(leiden_vals) else ""
                b = str(best_vals[i]) if best_vals and i < len(best_vals) else ""
                w.writerow([s, l, b, float(um[i,0]), float(um[i,1])])
        print(f"[{ts()}] Saved CSV: {out_path}")
        return True
    except Exception as _e:
        print(f"[{ts()}] Warning: failed to write {out_path}: {_e}")
...
def _obs_to_dataframe(adata):
    import pandas as pd
    _obs = getattr(adata, "obs", None)
    try:
        return pd.DataFrame(_obs.copy())
    except Exception:
        pass
    try:
        keys = []
...
def _export_umap_full(adata, out_path: str):
    import os, numpy as np
    import pandas as pd
    try:
        os.makedirs(os.path.dirname(out_path), exist_ok=True)
        df = _obs_to_dataframe(adata)
        ...
        print(f"[{ts()}] Warning: failed to write {out_path}: {e}")
        return False

def _obs_to_full_dataframe(adata):
    """Return a pandas DataFrame of ALL obs columns, robust to PyDataFrame backends."""
    ...
    try:
        ...
        try:
            ...
            pass
        for k in ["sample","leiden","n_fragment","passed_filters","majority","best_label",
                  "pred_cell_type","clusterName","leiden_majority_celltype","leiden_majority_clusterName"]:
            try:
                if k not in keys:
                    _ = obs[k] if hasattr(obs, "__getitem__") else getattr(obs, k)
                    keys.append(k)
            except Exception:
                pass
        d = {}
        ...
        return {"__manual_df__": True, "__keys__": list(d.keys()), "__data__": d, "__cells__": cells, "__rows__": m}
    except Exception as e:
        raise e

def _export_umap_full_any(adata, out_path: str):
    """Export ALL obs columns + umap_x/umap_y using pandas if available, otherwise manual fallback."""
    import os, csv, numpy as np
    try:
        os.makedirs(os.path.dirname(out_path), exist_ok=True)
        # UMAP
        if "X_umap" not in adata.obsm:
            print(f"[{ts()}] Warning: X_umap missing; skip {out_path}")
            return False
        um = adata.obsm["X_umap"]
        if hasattr(um, "toarray"):
            um = um.toarray()
        um = np.asarray(um)
        n = um.shape[0]
        pack = _obs_to_full_dataframe(adata)
        if isinstance(pack, dict) and pack.get("__manual_df__"):
            keys = pack["__keys__"]
            data = pack["__data__"]
            cells = pack["__cells__"]
            rows = min(pack["__rows__"], n, len(cells))
            header = ["cell"] + keys + ["umap_x","umap_y"]
            with open(out_path, "w", newline="") as f:
                w = csv.writer(f)
                w.writerow(header)
                for i in range(rows):
                    ...
                    for k in keys:
                        vals = data.get(k, [])
                        row.append("" if i >= len(vals) else str(vals[i]))
                    row += [float(um[i,0]), float(um[i,1])]
                    w.writerow(row)
        else:
            df = pack  # DataFrame
            try:
                import pandas as pd  # no-op to ensure available
                df = df.copy()
                df["umap_x"] = um[:len(df), 0]
                df["umap_y"] = um[:len(df), 1]
                df.to_csv(out_path, index=False)
            except Exception as e:
                # Fallback to manual writer if pandas unexpectedly fails (only macOS)
                import csv
                header = list(df.columns) + ["umap_x","umap_y"]
                with open(out_path, "w", newline="") as f:
                    w = csv.writer(f)
                    w.writerow(header)
...
def main(args=None):
    ...
    if SAVE_UMAP:
        ...
        _export_umap_full_any(data, os.path.join(FIG_DIR, "umap.csv"))
    ...
    if MAKE_GENE_ACTIVITY:
        try:
            ...
            try:
                ...
                gene_matrix = build_gene_activity_fallback(data, auto_gtf, flank_up=TSS_FLANK_UP, flank_down=TSS_FLANK_DOWN)
...
def _make_report_after_exit():
    try:
        ...
        base_dir = os.path.dirname(AGGR_DIR)
        summary_path = os.path.join(AGGR_DIR, "summary.txt")
        ...
        try:
            if _ad is not None and "X_umap" in _ad.obsm:
                ...
                try:
                    if "depth" in _ad.obs:
                        _umap["depth"] = _ad.obs["depth"].astype(float).tolist()
                    elif "n_fragment" in _ad.obs:
                        _umap["depth"] = _ad.obs["n_fragment"].astype(float).tolist()
                    else:
        ...
        _heat = {}
        try:
            _csv = os.path.join(FIG_DIR, "heatmap_robustz.csv")
